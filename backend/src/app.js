...
export function createJuloBackend({
  pool,
  secureCookies = true,
  ...
  const repository = Object.assign(
    createPostgresRepository(pool),
    createPostgresProjectNotesRepository(pool),
  );
  ...
  const projectNotesService = createProjectNotesService(repository);
