  ...
  return {
    async createProject({ id, workspaceId, name, actorUserId }) {
      return withTransaction(pool, async (client) => {
        const result = await client.query(
          ...
          [id, workspaceId, name, actorUserId],
        );
        await client.query(
          `INSERT INTO audit_events (workspace_id, actor_user_id, action, entity_type, entity_id, metadata)
           VALUES ($1,$2,'project.created','project',$3,$4::jsonb)`,
          [workspaceId, actorUserId, id, JSON.stringify({ name })],
        );
    ...
    async renameProject({ workspaceId, projectId, name, actorUserId }) {
      return withTransaction(pool, async (client) => {
        ...
        await client.query(
          `INSERT INTO audit_events (workspace_id, actor_user_id, action, entity_type, entity_id, metadata)
           VALUES ($1,$2,'project.renamed','project',$3,$4::jsonb)`,
          [workspaceId, actorUserId, projectId, JSON.stringify({ name })],
        );
        return result.rows[0];
      });
    },
    ...
    async archiveProject({ workspaceId, projectId, actorUserId }) {
      return withTransaction(pool, async (client) => {
        ...
        await client.query(
          `UPDATE tasks
           SET project_id = NULL, updated_at = now(), version = version + 1
           WHERE workspace_id = $1 AND project_id = $2`,
          [workspaceId, projectId],
        );
        await client.query(
          'DELETE FROM project_memberships WHERE workspace_id = $1 AND project_id = $2',
          [workspaceId, projectId],
        );
        await client.query(
          `INSERT INTO audit_events (workspace_id, actor_user_id, action, entity_type, entity_id, metadata)
           VALUES ($1,$2,'project.archived','project',$3,$4::jsonb)`,
          [workspaceId, actorUserId, projectId, JSON.stringify({ movedTasksToUnassigned: true })],
        );
        return result.rows[0];
      });
    },
    ...
    async setProjectMembership({ workspaceId, projectId, userId, role, actorUserId }) {
      return withTransaction(pool, async (client) => {
        ...
        if (!member.rows[0] || member.rows[0].status !== 'active') {
        ...
        const result = await client.query(
          `INSERT INTO project_memberships (project_id, workspace_id, user_id, role)
           VALUES ($1,$2,$3,$4)
           ON CONFLICT (project_id, user_id) DO UPDATE
           SET role = EXCLUDED.role,
               updated_at = now()
           RETURNING project_id, workspace_id, user_id, role`,
          [projectId, workspaceId, userId, role],
        );
        await client.query(
          `INSERT INTO audit_events (workspace_id, actor_user_id, action, entity_type, entity_id, metadata)
           VALUES ($1,$2,'project.member_assigned','project',$3,$4::jsonb)`,
          [workspaceId, actorUserId, projectId, JSON.stringify({ userId, projectRole: role })],
        );
    ...
    async removeProjectMembership({ workspaceId, projectId, userId, actorUserId }) {
      return withTransaction(pool, async (client) => {
        ...
        await client.query(
          `INSERT INTO audit_events (workspace_id, actor_user_id, action, entity_type, entity_id, metadata)
           VALUES ($1,$2,'project.member_removed','project',$3,$4::jsonb)`,
          [workspaceId, actorUserId, projectId, JSON.stringify({ userId })],
        );
        return result.rows[0];
      });
    },
    ...
    async listNotes(workspaceId) {
      const result = await pool.query(
        `SELECT id,
         ...
         ORDER BY status, reminder_date, updated_at DESC, id`,
        [workspaceId],
      );
      return result.rows;
    },
    ...
    async createNote({ id, workspaceId, title, description, reminderDate, label, actorUserId }) {
      return withTransaction(pool, async (client) => {
        const result = await client.query(
          `INSERT INTO notes (
             id, workspace_id, title, description, reminder_date, label, created_by
           )
           VALUES ($1,$2,$3,$4,$5,$6,$7)
           RETURNING *`,
          [id, workspaceId, title, description, reminderDate, label, actorUserId],
        );
        await client.query(
          `INSERT INTO audit_events (workspace_id, actor_user_id, action, entity_type, entity_id, metadata)
           VALUES ($1,$2,'note.created','note',$3,'{}'::jsonb)`,
          [workspaceId, actorUserId, id],
        );
        return result.rows[0];
      });
    },
        ...
        const result = await client.query(
          `UPDATE notes
           ...
           WHERE workspace_id = $1
             ...
             AND status = 'active'
           RETURNING *`,
          [workspaceId, noteId, expectedVersion, title, description, reminderDate, label],
        );
        if (!result.rows[0]) {
          const current = await client.query(
            'SELECT id, version, status FROM notes WHERE workspace_id = $1 AND id = $2',
            [workspaceId, noteId],
          );
          return current.rows[0]
            ? { kind: 'conflict', current: current.rows[0] }
            : { kind: 'not_found' };
        }
        await client.query(
          `INSERT INTO audit_events (workspace_id, actor_user_id, action, entity_type, entity_id, metadata)
           VALUES ($1,$2,'note.updated','note',$3,$4::jsonb)`,
          [workspaceId, actorUserId, noteId, JSON.stringify({ expectedVersion })],
        );
    ...
    async deleteNote({ workspaceId, noteId, actorUserId }) {
      return withTransaction(pool, async (client) => {
        ...
        await client.query(
          `INSERT INTO audit_events (workspace_id, actor_user_id, action, entity_type, entity_id, metadata)
           VALUES ($1,$2,'note.deleted','note',$3,'{}'::jsonb)`,
          [workspaceId, actorUserId, noteId],
        );
        return result.rows[0];
      });
    },
    ...
    async convertNoteToTask({
      ...
      return withTransaction(pool, async (client) => {
        ...
        const noteResult = await client.query(
          'SELECT * FROM notes WHERE workspace_id = $1 AND id = $2 FOR UPDATE',
          [workspaceId, noteId],
        );
        const note = noteResult.rows[0];
        if (!note) return { kind: 'not_found' };
        if (note.status !== 'active' || note.converted_task_id) {
          return { kind: 'already_converted', taskId: note.converted_task_id };
        }
        if (Number(note.version) !== Number(expectedVersion)) {
        ...
        const tags = note.label ? [note.label] : [];
        const taskResult = await client.query(
          `INSERT INTO tasks (
             id, workspace_id, project_id, title, description, status,
             execution_type, due_date, priority, assignee_user_id, created_by, tags, source_note_id
           )
        ...
        await client.query(
          `INSERT INTO audit_events (workspace_id, actor_user_id, action, entity_type, entity_id, metadata)
           VALUES ($1,$2,'note.converted_to_task','note',$3,$4::jsonb)`,
          [
            workspaceId,
            actorUserId,
            noteId,
