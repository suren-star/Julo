...
function taskMutationTools(client, { workspaceId, taskId, userId }) {
  return {
    async updateTask(changes, expectedVersion, now) {
      const columns = { title:'title', description:'description', executionType:'execution_type', dueDate:'due_date', priority:'priority', status:'status' };
    ...
    async pauseOtherRunningTimers(now) {
      await client.query(
        `UPDATE task_timers
         SET elapsed_ms = elapsed_ms + GREATEST(0, FLOOR(EXTRACT(EPOCH FROM ($3::timestamptz - started_at)) * 1000))::bigint,
             state = 'paused', started_at = NULL, updated_at = $3
         WHERE workspace_id = $1 AND task_id <> $2 AND state = 'running'`,
        [workspaceId, taskId, now]);
    },

    async startTimer(now) {
      const result = await client.query(
        `INSERT INTO task_timers (task_id, workspace_id, state, elapsed_ms, started_at, stopped_at, stop_reason, updated_at)
         VALUES ($1,$2,'running',0,$3,NULL,NULL,$3)
         ON CONFLICT (task_id) DO UPDATE
         SET state = 'running', started_at = EXCLUDED.started_at, stopped_at = NULL, stop_reason = NULL, updated_at = EXCLUDED.updated_at
         WHERE task_timers.state IN ('idle','paused')
         RETURNING *`, [taskId, workspaceId, now]);
      ...
      const result = await client.query(
        `UPDATE task_timers
         SET elapsed_ms = elapsed_ms + GREATEST(0, FLOOR(EXTRACT(EPOCH FROM ($3::timestamptz - started_at)) * 1000))::bigint,
             state = 'paused', started_at = NULL, updated_at = $3
         WHERE workspace_id = $1 AND task_id = $2 AND state = 'running'
         RETURNING *`, [workspaceId, taskId, now]);
      return mapTimer(result.rows[0]);
    },
    ...
    async stopTimer(now, reason) {
      const result = await client.query(
        `UPDATE task_timers
         SET elapsed_ms = elapsed_ms + CASE WHEN state = 'running'
               THEN GREATEST(0, FLOOR(EXTRACT(EPOCH FROM ($3::timestamptz - started_at)) * 1000))::bigint ELSE 0 END,
             state = 'stopped', started_at = NULL, stopped_at = $3, stop_reason = $4, updated_at = $3
         WHERE workspace_id = $1 AND task_id = $2 AND state IN ('running','paused')
         RETURNING *`, [workspaceId, taskId, now, reason]);
      return mapTimer(result.rows[0]);
    },

    async completeTimer(now) {
      const existing = await client.query(
        `SELECT * FROM task_timers WHERE workspace_id = $1 AND task_id = $2 FOR UPDATE`, [workspaceId, taskId]);
      const timer = existing.rows[0];
      if (timer?.state === 'stopped' && timer.stop_reason === 'manual') return mapTimer(timer);
      if (!timer) {
        const inserted = await client.query(
          `INSERT INTO task_timers (task_id, workspace_id, state, elapsed_ms, stopped_at, stop_reason, updated_at)
           VALUES ($1,$2,'stopped',0,$3,'completed',$3) RETURNING *`, [taskId, workspaceId, now]);
      ...
      const result = await client.query(
        `UPDATE task_timers
         SET elapsed_ms = elapsed_ms + CASE WHEN state = 'running'
               THEN GREATEST(0, FLOOR(EXTRACT(EPOCH FROM ($3::timestamptz - started_at)) * 1000))::bigint ELSE 0 END,
             state = 'stopped', started_at = NULL, stopped_at = $3, stop_reason = 'completed', updated_at = $3
         WHERE workspace_id = $1 AND task_id = $2 RETURNING *`, [workspaceId, taskId, now]);
      return mapTimer(result.rows[0]);
    },
    ...
    async reopenTimer(now) {
      const result = await client.query(
        `INSERT INTO task_timers (task_id, workspace_id, state, elapsed_ms, started_at, stopped_at, stop_reason, updated_at)
         ...
         ON CONFLICT (task_id) DO UPDATE
         SET elapsed_ms = task_timers.elapsed_ms + CASE WHEN task_timers.state = 'running'
               THEN GREATEST(0, FLOOR(EXTRACT(EPOCH FROM ($3::timestamptz - task_timers.started_at)) * 1000))::bigint ELSE 0 END,
             state = 'paused', started_at = NULL, stopped_at = NULL, stop_reason = NULL, updated_at = $3
         RETURNING *`, [taskId, workspaceId, now]);
      return mapTimer(result.rows[0]);
    },
  ...
  return {
    async withTaskMutationContext({ userId, workspaceId, taskId }, callback) {
      return withTransaction(pool, async (client) => {
        const workspaceLock = await client.query(`SELECT id FROM workspaces WHERE id = $1 FOR UPDATE`, [workspaceId]);
        const tx = taskMutationTools(client, { workspaceId, taskId, userId });
        if (!workspaceLock.rows[0]) return callback({ membership:null, task:null, projectRole:null, timer:null, tx });
        ...
        if (!membership || membership.status !== 'active') return callback({ membership, task:null, projectRole:null, timer:null, tx });
        ...
        if (task && membership.role === 'guest' && task.project_id) {
          const projectMembership = await client.query(
            `SELECT role FROM project_memberships
             WHERE workspace_id = $1 AND project_id = $2 AND user_id = $3 FOR SHARE`,
            [workspaceId, task.project_id, userId]);
          projectRole = projectMembership.rows[0]?.role ?? null;
        }
        let timer = null;
        if (task) {
          const timerResult = await client.query(`SELECT * FROM task_timers WHERE workspace_id = $1 AND task_id = $2 FOR UPDATE`, [workspaceId, taskId]);
